...
module.exports = (env) ->
	...
	registerWs: () ->
		...
		env.server.get '/api/apps/:key', env.middlewares.auth.needAccess('read'), (req, res, next) =>
			env.debug '/api/apps/'+req.params.key
			async.parallel [
		...
		env.server.post '/api/apps/:key', env.middlewares.auth.needAccess('update'), (req, res, next) =>
		...
		env.server.post '/api/apps/:key/reset', env.middlewares.auth.needAccess('update'), (req, res, next) =>
		...
		env.server.post '/api/apps/:key/keysets/:provider', env.middlewares.auth.needAccess('update'), (req, res, next) =>
		...
		env.server.del '/api/apps/:key/keysets/:provider', env.middlewares.auth.needAccess('update'), (req, res, next) =>
			env.data.apps.remKeyset req.params.key, req.params.provider, env.send(res,next)
		
		# get team settings for an app
		env.server.get '/api/apps/:key/access', env.middlewares.auth.needAccess('read'), (req, res, next) =>
			env.data.apps.getAccessList req.params.key, env.send(res,next)
		
		# set a user access in team settings for an app
		env.server.post '/api/apps/:key/access', env.middlewares.auth.needed, (req, res, next) =>
			env.data.apps.setAccess req.params.key, req.body?.id, req.body?.access, env.send(res,next)
		...
		env.server.get '/api/apps/:key/backend', env.middlewares.auth.needAccess('read'), (req, res, next) =>
			env.data.apps.getBackend req.params.key, env.send(res,next)

		# set or update the backend for an app
		env.server.post '/api/apps/:key/backend/:backend', env.middlewares.auth.needAccess('update'), (req, res, next) =>
		...
		env.server.del '/api/apps/:key/backend', env.middlewares.auth.needAccess('update'), (req, res, next) =>
			env.data.apps.remBackend req.params.key, env.send(res,next)

		# get the app's options
		env.server.get '/api/apps/:key/options', env.middlewares.auth.needAccess('read'), (req, res, next) =>
			env.data.apps.getOptions req.params.key, env.send(res, next)

		# set the app's options
		env.server.post '/api/apps/:key/options', env.middlewares.auth.needAccess('update'), (req, res, next) =>
